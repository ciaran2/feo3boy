//! Tests for the microcode executor.
#![cfg(test)]
...
fn test_loads_and_alu() {
    ...
    let mut state = MicrocodeState::default();
...
fn load8bit_immediate() {
    ...
    {
        for input in 0..=0xff {
            let mut ctx = (Gbz80State::default(), [0u8; 3], MicrocodeState::default());
            // Start with HL at a high number so any single-byte change to it will never
            // collide with the instruction at 0x00.
            ctx.0.regs.set_hl(2);
            ctx.1[0] = inst;
...
fn add8bit() {
    ...
    for existing_flags in (0x00..=0xf0).step_by(0x10) {
        for inst in 0x80..=0x86 {
            for v1 in 0..=0xff {
                for v2 in 0..=0xff {
                    ...
                    let mut ctx = (Gbz80State::default(), [0u8; 2], MicrocodeState::default());
                    ctx.1[0] = inst;
                    ctx.0.regs.acc = v1;
        ...
        for v in 0..=0xff {
            println!("add(87) {},{}", v, v);
            let mut ctx = (Gbz80State::default(), [0u8; 1], MicrocodeState::default());
            ctx.1[0] = 0x87;
            ctx.0.regs.acc = v;
            ctx.0.regs.flags = Flags::from_bits_truncate(existing_flags);
