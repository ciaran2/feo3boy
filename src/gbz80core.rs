...
fn load8reg(cpustate: &mut Gbz80state, mmu: &mut dyn MemDevice, toregnum: u8, fromregnum: u8) -> (u64, u8, u8) {
...
fn add(cpustate: &mut Gbz80state, mmu: &mut dyn MemDevice, regnum: u8) -> (u64, u8, u8) {
  ...
  if 0 == result {flags |= FZERO;}
  if 0 != (result & 0xff) {flags |= FCARRY;}
  if 0 != (((a & 0xf) + (reg & 0xf)) & 0x10) {flags |= FHALFCARRY;}
...
fn sub(cpustate: &mut Gbz80state, mmu: &mut dyn MemDevice, regnum: u8) -> (u64, u8, u8) {
  ...
  if 0 == result {flags |= FZERO;}
  if reg > a {flags |= FCARRY;}
  if (reg & 0xf) > (a & 0xf) {flags |= FHALFCARRY;}
...
fn subc(cpustate: &mut Gbz80state, mmu: &mut dyn MemDevice, regnum: u8) -> (u64, u8, u8) {
  ...
  let result = (Wrapping(a) - Wrapping(reg) - Wrapping(if check_flag(cpustate, FCARRY) {1u8} else {0u8})).0;
...
fn and(cpustate: &mut Gbz80state, mmu: &mut dyn MemDevice, regnum: u8) -> (u64, u8, u8) {
  ...
  let result = a & reg;
  let flags = if 0 == result {FZERO | FHALFCARRY} else {FHALFCARRY};
  cpustate.regs.regs816.write8(0o7, result);
  (4 + cycle_offset, flags, FALL)
}
...
mod tests {
  ...
  fn test_loads_and_alu() {
    ...
    assert_eq!(cpustate.regs.regs816.read8(0b111),0x80,"{:?}",cpustate.regs.regs816);
  
    tick(&mut cpustate, &mut testmem);
  
    assert_eq!(cpustate.regs.regs816.read8(0x0),0x01,"{:?}",cpustate.regs.regs816);
  
    tick(&mut cpustate, &mut testmem);
  
    assert_eq!(cpustate.regs.regs816.read8(0b111),0x81,"{:?}",cpustate.regs.regs816);

    tick(&mut cpustate, &mut testmem);

    assert_eq!(cpustate.regs.regs816.read8(0b001),0x85,"{:?}",cpustate.regs.regs816);

    tick(&mut cpustate, &mut testmem);

    assert_eq!(cpustate.regs.regs816.read8(0b111),0x6,"{:?}",cpustate.regs.regs816);
    assert_eq!(check_flag(&cpustate, FCARRY),true,"{:?}",cpustate.regs.regs816);
  }
}
